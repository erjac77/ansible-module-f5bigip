...
DOCUMENTATION = '''
---
module: f5bigip_ltm_monitor_postgresql
short_description: BIG-IP ltm monitor postgresql module
description:
...
BIGIP_LTM_MONITOR_POSTGRESQL_ARGS = dict(
...
class F5BigIpLtmMonitorPostgresql(F5BigIpNamedObject):
    def set_crud_methods(self):
        self.methods = {
            'create':   self.mgmt_root.tm.ltm.monitor.postgresqls.postgresql.create,
            'read':     self.mgmt_root.tm.ltm.monitor.postgresqls.postgresql.load,
            'update':   self.mgmt_root.tm.ltm.monitor.postgresqls.postgresql.update,
            'delete':   self.mgmt_root.tm.ltm.monitor.postgresqls.postgresql.delete,
            'exists':   self.mgmt_root.tm.ltm.monitor.postgresqls.postgresql.exists
        }
...
def main():
    ...
    module = AnsibleModuleF5BigIpNamedObject(argument_spec=BIGIP_LTM_MONITOR_POSTGRESQL_ARGS, supports_check_mode=False)

    try:
        obj = F5BigIpLtmMonitorPostgresql(check_mode=module.supports_check_mode, tr=tr, **module.params)
        result = obj.flush()
