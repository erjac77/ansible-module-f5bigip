    ...
    hash_session_dir:
        description:
            - Specifies whether the Microsoft terminal services are configured with a session directory, so the system
              does not load balance the initial connection.
        default: yes
        choices: ['yes', 'no']
    match_across_pools:
        description:
...
class F5BigIpLtmPersistenceMsrdp(F5BigIpNamedObject):
    def _set_crud_methods(self):
        self._methods = {
            'create': self._api.tm.ltm.persistence.msrdps.msrdp.create,
            'read': self._api.tm.ltm.persistence.msrdps.msrdp.load,
            'update': self._api.tm.ltm.persistence.msrdps.msrdp.update,
            'delete': self._api.tm.ltm.persistence.msrdps.msrdp.delete,
            'exists': self._api.tm.ltm.persistence.msrdps.msrdp.exists
        }
