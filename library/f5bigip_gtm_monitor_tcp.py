...
BIGIP_GTM_MONITOR_TCP_ARGS = dict(
...
class F5BigIpGtmMonitorTcp(F5BigIpNamedObject):
    def set_crud_methods(self):
        self.methods = {
            'create': self.mgmt_root.tm.gtm.monitor.tcps.tcp.create,
            'read': self.mgmt_root.tm.gtm.monitor.tcps.tcp.load,
            'update': self.mgmt_root.tm.gtm.monitor.tcps.tcp.update,
            'delete': self.mgmt_root.tm.gtm.monitor.tcps.tcp.delete,
            'exists': self.mgmt_root.tm.gtm.monitor.tcps.tcp.exists
        }


def main():
    module = AnsibleModuleF5BigIpNamedObject(argument_spec=BIGIP_GTM_MONITOR_TCP_ARGS, supports_check_mode=False)

    try:
        obj = F5BigIpGtmMonitorTcp(check_mode=module.supports_check_mode, **module.params)
        result = obj.flush()
