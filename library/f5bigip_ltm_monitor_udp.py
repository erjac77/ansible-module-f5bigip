...
BIGIP_LTM_MONITOR_UDP_ARGS = dict(
...
class F5BigIpLtmMonitorUdp(F5BigIpNamedObject):
    def set_crud_methods(self):
        self.methods = {
            'create':   self.mgmt_root.tm.ltm.monitor.udps.udp.create,
            'read':     self.mgmt_root.tm.ltm.monitor.udps.udp.load,
            'update':   self.mgmt_root.tm.ltm.monitor.udps.udp.update,
            'delete':   self.mgmt_root.tm.ltm.monitor.udps.udp.delete,
            'exists':   self.mgmt_root.tm.ltm.monitor.udps.udp.exists
        }
...
def main():
    ...
    module = AnsibleModuleF5BigIpNamedObject(argument_spec=BIGIP_LTM_MONITOR_UDP_ARGS, supports_check_mode=False)

    try:
        obj = F5BigIpLtmMonitorUdp(check_mode=module.supports_check_mode, tr=tr, **module.params)
        result = obj.flush()
