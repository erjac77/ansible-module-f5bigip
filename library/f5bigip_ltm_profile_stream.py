...
class F5BigIpLtmProfileStream(F5BigIpNamedObject):
    def _set_crud_methods(self):
        self._methods = {
            'create': self._api.tm.ltm.profile.streams.stream.create,
            'read': self._api.tm.ltm.profile.streams.stream.load,
            'update': self._api.tm.ltm.profile.streams.stream.update,
            'delete': self._api.tm.ltm.profile.streams.stream.delete,
            'exists': self._api.tm.ltm.profile.streams.stream.exists
        }
...
def main():
    ...
    try:
        obj = F5BigIpLtmProfileStream(check_mode=module.check_mode, **module.params)
        result = obj.flush()
